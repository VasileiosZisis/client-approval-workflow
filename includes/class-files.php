...
class Files
{
	...
	public function register_post_type()
	{
		register_post_type(
			...
			array(
				'labels'              => array(
					'name'          => __('Files', 'signoffflow'),
					'singular_name' => __('File', 'signoffflow'),
					'menu_name'     => __('Files', 'signoffflow'),
					'add_new_item'  => __('Add File', 'signoffflow'),
					'edit_item'     => __('Edit File', 'signoffflow'),
					'new_item'      => __('New File', 'signoffflow'),
					'view_item'     => __('View File', 'signoffflow'),
					'search_items'  => __('Search Files', 'signoffflow'),
					'not_found'     => __('No files found.', 'signoffflow'),
				),
				'public'              => false,
				'show_ui'             => true,
	...
	public function render_file_details_meta_box($post)
	{
		...
		?>
		<p>
			<label for="cliapwo_file_client_id"><strong><?php esc_html_e('Client', 'signoffflow'); ?></strong></label><br />
			<select
				class="widefat"
		...
		<p>
			<label for="cliapwo_file_upload"><strong><?php esc_html_e('Upload file', 'signoffflow'); ?></strong></label><br />
			<input
				type="file"
		...
		<?php if ($has_stored_file) : ?>
			<p>
				<strong><?php esc_html_e('Current file', 'signoffflow'); ?>:</strong>
				<?php echo esc_html('' !== $original_file_name ? $original_file_name : basename($stored_file_path)); ?>
			</p>
		<?php elseif ('' !== $stored_relative) : ?>
			<p class="description"><?php esc_html_e('A protected file is referenced for this record, but it is missing from storage.', 'signoffflow'); ?></p>
		<?php else : ?>
			<p class="description"><?php esc_html_e('No file uploaded yet.', 'signoffflow'); ?></p>
		<?php endif; ?>

		<p class="description">
			<?php esc_html_e('Allowed file types follow your WordPress upload settings. Uploading a new file replaces the current file for this record.', 'signoffflow'); ?>
		</p>
		<p class="description">
			<?php esc_html_e('Client downloads always go through SignoffFlow access checks. On Nginx hosts, add a matching server deny rule for the protected uploads directory.', 'signoffflow'); ?>
		</p>
		<?php
	}
	...
	{
		$columns['cliapwo_file_client'] = __('Client', 'signoffflow');
		$columns['cliapwo_file_name']   = __('Stored file', 'signoffflow');
	...
	public function render_file_column($column, $post_id)
	{
		if ('cliapwo_file_client' === $column) {
			...
			echo $client instanceof \WP_Post ? esc_html($client->post_title) : esc_html__('Unassigned', 'signoffflow');
			return;
		}
		...
		echo '' !== (string) $file_name ? esc_html((string) $file_name) : esc_html__('No file uploaded', 'signoffflow');
	}
	...
	public function handle_download()
	{
		...
		if (! wp_verify_nonce($nonce, self::DOWNLOAD_ACTION)) {
			wp_die(
				esc_html__('Invalid download request.', 'signoffflow'),
		...
		if (! is_user_logged_in()) {
			wp_die(
				esc_html__('You must be logged in to download files.', 'signoffflow'),
				esc_html__('Forbidden', 'signoffflow'),
				array(
					'response' => 403,
				)
		...
		if (! current_user_can('cliapwo_manage_portal') && ! current_user_can('cliapwo_view_portal')) {
			wp_die(
				esc_html__('You are not allowed to download files.', 'signoffflow'),
				esc_html__('Forbidden', 'signoffflow'),
				array(
					'response' => 403,
				)
		...
		if (! $file_post instanceof \WP_Post || self::POST_TYPE !== $file_post->post_type || 'publish' !== $file_post->post_status) {
			wp_die(
				esc_html__('The requested file could not be found.', 'signoffflow'),
				esc_html__('Not Found', 'signoffflow'),
				array(
					'response' => 404,
				)
			);
		...
		if (! Clients::user_can_view_client($client_id, get_current_user_id())) {
			wp_die(
				esc_html__('You do not have access to this file.', 'signoffflow'),
				esc_html__('Forbidden', 'signoffflow'),
				array(
					'response' => 403,
				)
		...
		if ('' === $file_path || ! file_exists($file_path)) {
			wp_die(
				esc_html__('The requested file is missing from protected storage.', 'signoffflow'),
				esc_html__('Not Found', 'signoffflow'),
				array(
					'response' => 404,
				)
	...
	private function store_uploaded_file(array $file_data)
	{
		if (UPLOAD_ERR_OK !== (int) $file_data['error']) {
			return new \WP_Error(
				...
				__('The file upload failed. Please try again.', 'signoffflow')
			);
		}
		...
		if ('' === $original_name || '' === $tmp_name) {
			return new \WP_Error(
				...
				__('The uploaded file data is incomplete.', 'signoffflow')
			);
		}
		...
		if (! is_uploaded_file($tmp_name)) {
			return new \WP_Error(
				...
				__('The uploaded file could not be validated.', 'signoffflow')
			);
		}
		...
		if ('' === $extension || '' === $mime_type || ! in_array($mime_type, $allowed_mimes, true)) {
			return new \WP_Error(
				...
				__('That file type is not allowed.', 'signoffflow')
			);
		}
		...
		if (! $move_succeeded || ! file_exists($destination_path)) {
			return new \WP_Error(
				...
				__('The uploaded file could not be moved into protected storage.', 'signoffflow')
			);
		}
	...
	private static function ensure_storage_directory()
	{
		...
		if (! is_dir($storage_directory) && ! wp_mkdir_p($storage_directory)) {
			return new \WP_Error(
				...
				__('The protected storage directory could not be created.', 'signoffflow')
			);
		}
	...
	private static function get_storage_directory_path()
	{
		...
		if (! is_array($uploads) || ! empty($uploads['error']) || empty($uploads['basedir']) || ! is_string($uploads['basedir'])) {
			return new \WP_Error(
				...
				__('The uploads directory is not available for protected files.', 'signoffflow')
			);
		}
		...
		foreach ($hardening_files as $file_name => $file_contents) {
			...
			if (false === $bytes_written) {
				return new \WP_Error(
					...
					__('The protected storage directory could not be hardened.', 'signoffflow')
				);
			}
		}
